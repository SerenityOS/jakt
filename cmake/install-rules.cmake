
set(CMAKE_INSTALL_MESSAGE NEVER)

include(GNUInstallDirs)
include(CMakePackageConfigHelpers)

# Allow package maintainers to freely override the path for the configs
set(Jakt_INSTALL_CMAKEDIR "${CMAKE_INSTALL_DATADIR}/${package}"
    CACHE PATH "CMake package config location relative to the install prefix")
mark_as_advanced(Jakt_INSTALL_CMAKEDIR)

install(
    FILES cmake/install-config.cmake
    DESTINATION "${Jakt_INSTALL_CMAKEDIR}"
    RENAME JaktConfig.cmake
    COMPONENT Jakt_Development
)

install(
    IMPORTED_RUNTIME_ARTIFACTS jakt_stage1
    RUNTIME
        DESTINATION "${CMAKE_INSTALL_BINDIR}/stage1"
        COMPONENT Jakt_Runtime
)

set(stages stage2 stage3)
foreach (stage IN LISTS stages)
  set(target_name "jakt_${stage}")
  install(
    TARGETS ${target_name}
    EXPORT JaktTargets
    RUNTIME #
        DESTINATION "${CMAKE_INSTALL_BINDIR}/${stage}"
        COMPONENT Jakt_Runtime
    LIBRARY #
        DESTINATION "${CMAKE_INSTALL_LIBDIR}/${stage}"
        COMPONENT Jakt_Runtime
        NAMELINK_COMPONENT Jakt_Development
    ARCHIVE #
        DESTINATION "${CMAKE_INSTALL_LIBDIR}/${stage}"
        COMPONENT Jakt_Development
  )
endforeach()

# Make a symlink bin/jakt to the final compiler we created
set(final_stage stage3)
install(CODE "execute_process(COMMAND
                              ${CMAKE_COMMAND} -E create_symlink \
                                                  ${final_stage}/jakt_${final_stage} \
                                                  ${CMAKE_INSTALL_PREFIX}/${CMAKE_INSTALL_BINDIR}/jakt)"
)

# Install runtime files. In the future we should probably have a bazillion rules
#   for compiling this into a static archive for different configs
install(DIRECTORY "runtime"
        DESTINATION ${CMAKE_INSTALL_INCLUDEDIR}
        COMPONENT Jakt_Development
        FILES_MATCHING PATTERN "*.h"
                       PATTERN "*.cpp"
                       PATTERN "utility"
)

# FIXME: Figure out how to get an autogenerated import target for:
#   - stage 1 compiler, where we install(IMPORTED_RUNTIME_ARTIFACTS)
#   - final compiler (Jakt::jakt), which is the symlink above
install(
    EXPORT JaktTargets
    NAMESPACE Jakt::
    DESTINATION "${Jakt_INSTALL_CMAKEDIR}"
    COMPONENT Jakt_Development
)
