...
if (is_directory){
auto __jakt_var_133 = [&]() -> ErrorOr<void> { return TRY((jakt__platform__unknown_fs::make_directory(((((to).join(((path_relative_to_target).to_string())))).to_string())))), ErrorOr<void>{}; }();
;
((directories_to_copy).enqueue((Tuple{path, path_relative_to_target})));
continue;
}
auto __jakt_var_135 = [&]() -> ErrorOr<void> { return TRY((mkdir_p(((((to).join(((path_relative_to_target).to_string())))).parent())))), ErrorOr<void>{}; }();
;
...
size_t const max_concurrent = (infallible_integer_cast<size_t>((({ Optional<u32> __jakt_var_136;
auto __jakt_var_137 = [&]() -> ErrorOr<u32> { return ((value_or_throw<u32>(((compiler_job_count).template to_number<u32>())))); }();
...
return JaktInternal::ExplicitValue<JaktInternal::DynamicArray<types::Value>>(DynamicArray<types::Value>::create_with({types::Value(types::ValueImpl::JaktArray(passed_arguments,(((((first_main_param.value())).variable))->type_id)),call_span)}));
}
VERIFY_NOT_REACHED();
}
else if (__jakt_enum_value == false) {
...
jakt__path::Path const path = ((binary_dir).join(file));
auto __jakt_var_139 = [&]() -> ErrorOr<void> { return TRY((write_only_if_updated(contents,((path).to_string())))), ErrorOr<void>{}; }();
if (__jakt_var_139.is_error()) {auto error = __jakt_var_139.release_error();
{
...
if (((generate_depfile).has_value())){
auto __jakt_var_140 = [&]() -> ErrorOr<void> {{
TRY((write_only_if_updated(((depfile_builder).to_string()),(generate_depfile.value()))));
}

;return {};}();
if (__jakt_var_140.is_error()) {auto error = __jakt_var_140.release_error();{
...
auto __jakt_var_142 = [&]() -> ErrorOr<void> { return TRY((((builder).build_all(binary_dir,(([use_ccache, cxx_compiler_path, runtime_path, extra_include_paths, optimize, extra_compiler_flags](ByteString input_filename, ByteString output_filename) -> ErrorOr<JaktInternal::DynamicArray<ByteString>> {{
return ((platform__unknown_compiler::run_compiler(cxx_compiler_path,input_filename,output_filename,runtime_path,extra_include_paths,DynamicArray<ByteString>::create_with({}),DynamicArray<ByteString>::create_with({}),optimize,extra_compiler_flags,use_ccache)));
}
}
)))))), ErrorOr<void>{}; }();
if (__jakt_var_142.is_error()) {{
return static_cast<int>(1);
}
}
;
...
auto __jakt_var_144 = [&]() -> ErrorOr<void> { return TRY((((builder).link_into_archive(archiver_path.value_or_lazy_evaluated([&] { return (ByteString::from_utf8_without_validation("ar"sv)); }),((((binary_dir).join((link_archive.value())))).to_string()),extra_arguments)))), ErrorOr<void>{}; }();
...
auto __jakt_var_146 = [&]() -> ErrorOr<void> { return TRY((((builder).link_into_executable(cxx_compiler_path,output_filename,extra_arguments)))), ErrorOr<void>{}; }();
if (__jakt_var_146.is_error()) {{
return static_cast<int>(1);
}
}
;
...
{
JaktInternal::Optional<NonnullRefPtr<File>> maybe_file = ({ Optional<NonnullRefPtr<File>> __jakt_var_147;
auto __jakt_var_148 = [&]() -> ErrorOr<NonnullRefPtr<File>> { return ((File::open_for_reading(path))); }();
if (!__jakt_var_148.is_error()) __jakt_var_147 = __jakt_var_148.release_value();
__jakt_var_147; });
if (((maybe_file).has_value())){
JaktInternal::Optional<JaktInternal::DynamicArray<u8>> const contents = ({ Optional<JaktInternal::DynamicArray<u8>> __jakt_var_149;
auto __jakt_var_150 = [&]() -> ErrorOr<JaktInternal::DynamicArray<u8>> { return (((((maybe_file.value()))->read_all()))); }();
if (!__jakt_var_150.is_error()) __jakt_var_149 = __jakt_var_150.release_value();
__jakt_var_149; });
