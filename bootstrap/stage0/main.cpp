...
return JaktInternal::ExplicitValue(({ Optional<size_t> __jakt_var_773; {
JaktInternal::Optional<u32> const end_input = ((((parts)[static_cast<i64>(1LL)])).to_uint());
if ((!(((end_input).has_value())))){
return (JaktInternal::OptionalNone());
}
__jakt_var_773 = (infallible_integer_cast<size_t>(((end_input.value())))); goto __jakt_label_694;

}
__jakt_label_694:; __jakt_var_773.release_value(); }));
}
else {
{
return (JaktInternal::OptionalNone());
...
size_t const max_concurrent = (infallible_integer_cast<size_t>((({ Optional<u32> __jakt_var_774;
auto __jakt_var_775 = [&]() -> ErrorOr<u32> { return TRY((value_or_throw<u32>(((compiler_job_count).to_uint())))); }();
if (__jakt_var_775.is_error()) {{
warnln(Jakt::DeprecatedString("error: invalid value for --jobs: {}"sv),compiler_job_count);
return (static_cast<i64>(1LL));
}
} else {__jakt_var_774 = __jakt_var_775.release_value();
}
__jakt_var_774.release_value(); }))));
...
if (__jakt_enum_value == true) {
return JaktInternal::ExplicitValue(({ Optional<JaktInternal::DynamicArray<types::Value>> __jakt_var_776; {
...
__jakt_var_776 = (TRY((DynamicArray<types::Value>::create_with({types::Value(TRY((types::ValueImpl::template __jakt_create<typename types::ValueImpl::JaktArray>(passed_arguments,(((((first_main_param.value())).variable))->type_id)))),call_span)})))); goto __jakt_label_695;

}
__jakt_label_695:; __jakt_var_776.release_value(); }));
}
else {
...
auto __jakt_var_778 = [&]() -> ErrorOr<void> { return TRY((utility::write_to_file(contents,((path).to_string())))), ErrorOr<void>{}; }();
if (__jakt_var_778.is_error()) {auto error = __jakt_var_778.release_error();
{
...
auto __jakt_var_779 = [&]() -> ErrorOr<void> {{
TRY((utility::write_to_file(TRY((((depfile_builder).to_string()))),(generate_depfile.value()))));
}

;return {};}();
if (__jakt_var_779.is_error()) {auto error = __jakt_var_779.release_error();{
...
auto __jakt_var_781 = [&]() -> ErrorOr<void> { return TRY((((builder).build_all(binary_dir,(([cxx_compiler_path, runtime_path, extra_include_paths, optimize, extra_compiler_flags](DeprecatedString input_filename, DeprecatedString output_filename) -> ErrorOr<JaktInternal::DynamicArray<DeprecatedString>> {
{
...
if (__jakt_var_781.is_error()) {{
return (static_cast<i64>(1LL));
}
}
;
if (((link_archive).has_value())){
auto __jakt_var_783 = [&]() -> ErrorOr<void> { return TRY((((builder).link_into_archive(archiver_path.value_or_lazy_evaluated([&] { return Jakt::DeprecatedString("ar"sv); }),(link_archive.value()))))), ErrorOr<void>{}; }();
if (__jakt_var_783.is_error()) {{
return (static_cast<i64>(1LL));
}
}
;
...
auto __jakt_var_785 = [&]() -> ErrorOr<void> { return TRY((((builder).link_into_executable(cxx_compiler_path,output_filename,extra_arguments)))), ErrorOr<void>{}; }();
if (__jakt_var_785.is_error()) {{
return (static_cast<i64>(1LL));
}
}
;
