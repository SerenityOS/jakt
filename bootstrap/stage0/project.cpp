...
{
DeprecatedString const cml_contents = TRY((((TRY((((TRY((((TRY((((TRY(DeprecatedString::from_utf8("cmake_minimum_required(VERSION 3.20)\nproject("sv))) + (((*this).name)))))) + (TRY(DeprecatedString::from_utf8("\n   VERSION 1.0.0\n   LANGUAGES CXX\n)\n\nfind_package(Jakt REQUIRED)\n\nadd_jakt_executable("sv))))))) + (((*this).name)))))) + (TRY(DeprecatedString::from_utf8("\n   MAIN_SOURCE src/main.jakt\n   MODULE_SOURCES\n     src/second_module.jakt\n)\n"sv))))));
TRY((utility::write_to_file(cml_contents,TRY((((project_directory) + (TRY(DeprecatedString::from_utf8("/CMakeLists.txt"sv)))))))));
}
return {};
}
...
DeprecatedString const project_directory = TRY((((TRY((((current_directory) + (TRY(DeprecatedString::from_utf8("/"sv))))))) + (((*this).name)))));
...
TRY((jakt__platform__unknown_fs::make_directory(TRY((((project_directory) + (TRY(DeprecatedString::from_utf8("/src"sv)))))))));
...
{
DeprecatedString const main_jakt = TRY(DeprecatedString::from_utf8("import second_module { get_string }\n\nfn main() throws -> c_int {\n    println(\"{}!\", get_string())\n    return 0\n}\n"sv));
DeprecatedString const second_module_jakt = TRY(DeprecatedString::from_utf8("fn get_string() throws -> String {\n    return \"Hello, World\"\n}\n"sv));
TRY((utility::write_to_file(main_jakt,TRY((((project_directory) + (TRY(DeprecatedString::from_utf8("/src/main.jakt"sv)))))))));
TRY((utility::write_to_file(second_module_jakt,TRY((((project_directory) + (TRY(DeprecatedString::from_utf8("/src/second_module.jakt"sv)))))))));
}
return {};
}
