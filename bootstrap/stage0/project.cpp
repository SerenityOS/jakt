...
String const project_directory = ((current_directory + Jakt::String("/")) + ((*this).name));
outln(Jakt::String("Creating jakt project in {}.."),project_directory);
TRY((unknown_fs::make_directory(project_directory)));
TRY((unknown_fs::make_directory((project_directory + Jakt::String("/src")))));
out(Jakt::String("\tGenerating CMakeLists.txt..."));
TRY((((*this).create_template_cmake_lists(project_directory))));
outln(Jakt::String(" done"));
out(Jakt::String("\tGenerating jakt source code..."));
TRY((((*this).create_sample_jakt_files(project_directory))));
outln(Jakt::String(" done"));
out(Jakt::String("\tGenerating README.md..."));
...
{
String const cml_contents = ((((Jakt::String("cmake_minimum_required(VERSION 3.20)\nproject(") + ((*this).name)) + Jakt::String("\n   VERSION 1.0.0\n   LANGUAGES CXX\n)\n\nfind_package(Jakt REQUIRED)\n\nadd_jakt_executable(")) + ((*this).name)) + Jakt::String("\n   MAIN_SOURCE src/main.jakt\n   MODULE_SOURCES\n     src/second_module.jakt\n)\n"));
TRY((utility::write_to_file(cml_contents,(project_directory + Jakt::String("/CMakeLists.txt")))));
}
return {};
}
...
{
String const main_jakt = Jakt::String("import second_module { get_string }\n\nfunction main() throws -> c_int {\n    println(\"{}!\", get_string())\n    return 0\n}\n");
String const second_module_jakt = Jakt::String("function get_string() throws -> String {\n    return \"Hello, World\"\n}\n");
TRY((utility::write_to_file(main_jakt,(project_directory + Jakt::String("/src/main.jakt")))));
TRY((utility::write_to_file(second_module_jakt,(project_directory + Jakt::String("/src/second_module.jakt")))));
}
return {};
}
