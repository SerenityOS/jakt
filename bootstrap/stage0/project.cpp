...
{
DeprecatedString const cml_contents = ((((TRY(DeprecatedString::from_utf8("cmake_minimum_required(VERSION 3.20)\nproject("sv)) + ((*this).name)) + TRY(DeprecatedString::from_utf8("\n   VERSION 1.0.0\n   LANGUAGES CXX\n)\n\nfind_package(Jakt REQUIRED)\n\nadd_jakt_executable("sv))) + ((*this).name)) + TRY(DeprecatedString::from_utf8("\n   MAIN_SOURCE src/main.jakt\n   MODULE_SOURCES\n     src/second_module.jakt\n)\n"sv)));
TRY((utility::write_to_file(cml_contents,(project_directory + TRY(DeprecatedString::from_utf8("/CMakeLists.txt"sv))))));
}
return {};
}
...
DeprecatedString const project_directory = ((current_directory + TRY(DeprecatedString::from_utf8("/"sv))) + ((*this).name));
...
TRY((jakt__platform__unknown_fs::make_directory((project_directory + TRY(DeprecatedString::from_utf8("/src"sv))))));
...
TRY((utility::write_to_file(main_jakt,(project_directory + TRY(DeprecatedString::from_utf8("/src/main.jakt"sv))))));
TRY((utility::write_to_file(second_module_jakt,(project_directory + TRY(DeprecatedString::from_utf8("/src/second_module.jakt"sv))))));
}
return {};
}
