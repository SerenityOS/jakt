...
template<typename T>
struct Jakt::Traits<Jakt::NonnullRefPtr<T>> : public Jakt::GenericTraits<Jakt::NonnullRefPtr<T>> {
    using PeekType = T*;
    using ConstPeekType = const T*;
    static unsigned hash(Jakt::NonnullRefPtr<T> const& p) { return ptr_hash(p.ptr()); }
    static bool equals(Jakt::NonnullRefPtr<T> const& a, Jakt::NonnullRefPtr<T> const& b) { return a.ptr() == b.ptr(); }
};
#endif
